...
World data pulled from [Global.health](https://github.com/globaldothealth/monkeypox) `r Sys.time()`.  
...
tm_shape(us_latest_map)+
  tm_polygons(col = "cumulative_confirmed", breaks = c(0, 50, 100, 200, 300, 400, Inf))
...
shinyApp(ui, server1, options = list(height = 1000))
```
